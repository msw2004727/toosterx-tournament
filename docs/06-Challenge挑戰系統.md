...
> **2026-09-06 主辦修訂：挑戰卡改綁 LINE 帳號、由系統配發。** 玩家用 LINE 登入，
> Cloud Function `issuePlayerQr` 在交易裡配一組代號並寫進 `users/{uid}.gamePassId`，
> 暱稱先用 LINE 名稱。一個帳號一張，換手機用同一個帳號登入就是同一張，所以不再有
> 「取暱稱建卡」與「找回」。沒有 LINE 的小朋友由家長的帳號領一張，第二個孩子到攤位代建。
> 以下原文保留作為背景。
