...
- [x] 第二輪與第三輪驗收單核實（2026-09-06 晚間、09-07 凌晨）：家長／隊長、管理員、檢錄員、記錄員四張單，
...
主辦交來兩份獨立驗收報告（《驗收計畫書整合版》146 項、D-01…D-15；《Codex 獨立唯讀驗收》
重新分類並修正了四處敘述、新增 C-01）。逐條在 `b26ba3c` 上核實：**15 條屬實、只有 D-05 已不成立**
（雲端早就換成 `onAttemptWritten`）。完整的核實與處置表在 **`docs/13-驗收整合報告-2026-09-06.md`**。

這一輪值得記下的四件事：

1. **替身資料寫錯 schema 比沒有測試更危險——第五次。** 檢錄台讀頂層 `homeTeamId`，真實文件是巢狀
   `home.teamId`；E2E 種子照著錯的 schema 寫，所以檢錄台在測試裡一直是綠的、在真站上永遠 0/0。
   修法不只改程式，**種子裡不存在的欄位一定要拿掉**，不然測試證明不了任何事（變異 #E18）。
2. **「某一層有測」不等於「變異抓得到」——又一次。** 賽務首頁（D-13）多讀了一次 `matchsheet.write`
   之後，出場名單頁改讀別的權限（變異 #C10）就從 T42-8「有人讀」底下逃掉了：T42-8 只問有沒有人讀。
   現在 T58-G 另外釘住「權限碼要由擁有那個功能的頁面自己讀」。
3. **勝負只能有一份實作。** 賽務端完賽與管理端改判各算一份 result，PK 標記分岔（3:1＋PK 2:4 一邊
   標 penalty、一邊 regulation；Codex 抓到的 C-01）。合併進 `js/engine/result.js` 的 `matchResult()`。
...
`docs/lighthouse/` 放量測報告（`mobile.report.*` 修改前的正式站 66、`demo-mobile.report.*` 修改後的 demo 96、`mobile-after.report.*` 修改後的正式站 87）。
...
### 第二輪驗收單（2026-09-06 晚間）：先核實再修，非問題只回報

主辦交來家長／隊長與管理員兩張單，指示「先審核是否屬實再修正，只是操作誤會的回報就好」。
逐條對照**資料庫**核實（docs/13 §9 有完整表）。這一輪值得記下的：

1. **核實要看資料，不能只看畫面。** 「後四碼 1234 可以、9070 不行」在資料庫裡是四筆被
   `fn:onePlayerOneTeam` 退件的成員（同一個孩子已在另一隊待審）。退件是 Function 一秒後
   做的，名單頁上那一列直接消失，沒有任何解釋——測試人只能猜。**系統退件現在一定看得見**：
   名單頁列出原因、隊長可收掉（rules 放行 rejected → removed，R57c／RU#49）；加入頁先講
   「你已經有一筆待審」，送出後盯著那一筆把原因拿回頁上；「我的」的球員列附原因。
2. **測試人寫「環境：demo」，實際在正式站測。** 正式站留下 8 支測試球隊、3 場 U10 完賽場次、
   人工裁定過的積分榜，U10 賽程已發布。清理腳本 `scratchpad/prod-cleanup.mjs`（預設 dry-run）
   **要主辦確認才能跑**。
3. **「我的」原本連到公開球隊頁，而那一頁沒有任何路通往管理頁。** R-5／R-6／R-11 三條
   「找不到審核鈕／取消鈕」都是這個原因。現在「我的」直接進管理頁，公開球隊頁隊長看得到
   「管理名單／審核申請」。
4. **隊職員不問背號**（正式站的領隊掛著 20 號），球員撞號在「加入名單」時就擋，不等審核。
5. **草稿可以由隊長自己取消**（rules draft → withdrawn，R98e／R98f、RU#47／#48）；
   被退回的走「申請取消」。原本這兩種狀態沒有任何取消的路。
6. **稽核：正式站 87 筆裡 62 筆印成 `standing.manualRanking：積分榜 …`，而且不在任何分類。**
   `describeAudit` 補了 16 種動作的人話；分類改成 7 個，「其他」是補集，**加總等於全部**
   （T39-D 守）。分頁列不換行、橫向捲動（360px 會折成「報名審／核」）。
7. **`<input type="time">` 在 Android 跟著手機的 12／24 小時設定走，網頁改不掉。**
   賽程的開賽時間改成文字格，`normalizeHHMM` 收 `930`／`9:30`／`14.30`，存 24 小時制。
8. 後台返回鍵改成「從哪來回哪去」（`adminBack()`）；改判頁頭印「目前比分 2:2（PK 4:3）」；
   公開端比賽頁與賽程列印 `PK 4-3`（`js/lib/format.js` 的 `pkText`）；攤位加「代建新卡（系統配號）」；
   三態燈的白字只留給賽務端的深色頁首（攤位頁在淺色底上看不見）。
9. **操作誤會，只回報**：邀請連結 vs LINE 網址列的 `?lineAppVersion=`（順手把連結印出來並加
   「分享給隊友」）；demo 上「不能對調分組」是成人公開組已有場次開打、U10 與女子組是單一組別
   （介面現在兩種情況都會寫原因；部署後逐組核對過 U6／U8／興趣仍可對調）。

### 第三輪驗收單（2026-09-07 凌晨，檢錄員／記錄員）：先重現再修

主辦交來檢錄員（失敗 3）與記錄員（失敗 1、備註 1）兩張單，指示「確認是瑕疵再做修改，不要只照做」。
每一條先用 Playwright 在 demo 以**那個身分**重現，再對程式碼與資料庫（完整表在 docs/13 §10）。結論：

| 項 | 核實 |
|---|---|
| C-1 檢錄員可編出場名單 | **不屬實**。真的檢錄員只有「賽務台／檢錄」；三個選項是裁判以上。切換身分清單裡裁判那一列的說明以「檢錄員 ＋ …」開頭，測試人按錯列。說明改成「多了：…（其餘同 X）」 |
| C-3 有問題的人仍可勾出賽 | 屬實。出賽與有問題是同一個欄位，勾下去會悄悄把註記洗掉。現在鎖住，要先「取消註記」 |
| C-5 一個人也能完成檢錄 | **屬實而且更嚴重**：門檻讀 `checkin.requiredMin`，真實文件沒有這欄位；而且「完成檢錄」什麼都沒寫 |
| S-9 管理員好像還是唯讀 | 回到 S-7 剛鎖定的那一場。鎖定場次對所有人唯讀（含管理員），但提示叫管理員「去找管理員」。現在給他「到場次改判頁」的按鈕 |
| S-5 備註 換人選單不更新 | 屬實；核實時另外抓到 **`isPlayerRow` 不認 start／bench：出場名單一經確認，賽務台的球員選單一個人都不剩** |

#### 完成檢錄（C-5）現在是這樣的

```
js/modules/staff/checkin-actions.js  requiredMinOf（場次 requiredMin → 組別 minPlayersToStart → playersOnField，都沒有 → null）
                                     checkinGate（不足：檢錄員按不下去；管理員 can('checkin.force') 可放行但要原因；門檻 null 連管理員都不行）
                                     buildCheckinConfirmPatch（checkin 那一包整包寫回、狀態 scheduled→checkin→ready）
firestore.rules                      matches 的 (E) 分支：isCheckin 只准動 checkin／status／updatedAt／updatedBy，狀態只能到 checkin／ready
js/config.js                         新權限碼 checkin.force（群組「檢錄」，minRole admin）
```

⚠️ **`updateDoc` 對 `checkin` 這一包是整包取代**，另一隊的旗標要抄回去（#AF28、T41-6）。
⚠️ 門檻讀不到就不放行，管理員也一樣——那是設定壞了，要先修設定（#AF24）。

#### 這一輪值得記下的

1. **替身資料寫錯 schema，第六次。** `checkin: { requiredMin: 5 }` 在 checkin.spec 與 perm-effect.spec 各一份，
   真實文件從來沒有那個欄位；「人數不足說清楚」的 E2E 綠了三輪。
2. **「有 E2E 在測」的按鈕可以什麼都沒寫。** 完成檢錄的 E2E 只看畫面上的字；送出類的 E2E 一定要 dump 資料庫。
3. **操作誤會也要看 UI 有沒有引導它。** C-1 是按錯列，但那一列的說明就是用下一階的名字開頭的。
